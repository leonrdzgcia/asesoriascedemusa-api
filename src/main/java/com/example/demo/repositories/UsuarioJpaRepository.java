...
public interface UsuarioJpaRepository extends JpaRepository<UsuariosModel, Long> {
 ...
 @Query(value ="SELECT * FROM springdb.usuarios u where u.nombre like %:nombre%",nativeQuery = true)
 List<UsuariosModel> buscarNombre(String nombre);

 @Query(value ="SELECT * FROM springdb.usuarios u where u.matricula = ?1",nativeQuery = true)
 List<UsuariosModel>  buscarMatricula(String matricula);

 @Query(value ="SELECT * FROM springdb.usuarios u where u.id_usuario = ?1",nativeQuery = true)
 ...
 @Query(value = "select * from usuario c where upper(c.nombre) like upper(concat('%', ?1,'%'))", nativeQuery = true)
